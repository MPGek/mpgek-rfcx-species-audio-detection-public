...
from model.random_resized_crop import RandomResizedCrop
...
def gauss_noise(image: np.ndarray, sigma_sq):
    h, w = image.shape
    gauss = np.random.normal(0, sigma_sq, (h, w)).astype(image.dtype)
...
class PreprocessMelImage:
    def __call__(self, image):
        delta = librosa.feature.delta(image)
        accelerate = librosa.feature.delta(image, order=2)
        # image = np.stack([image, delta, accelerate], axis=0)
        image = np.stack([0.01 * image, 0.1 * delta, 0.1 * accelerate], axis=0)
        # image = image.astype(np.float32) / 100
        # image = torch.from_numpy(image)
        return image


class PreprocessSingleChannelMelImage:
...
class RandomCrop:
    ...
    def __call__(self, signal):
        ...
        return signal[:, start: start + self.size], start
